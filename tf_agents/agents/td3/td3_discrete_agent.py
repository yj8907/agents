...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 31 18:57:03 2019
...
@gin.configurable
class Td3DiscreteAgent(tf_agent.TFAgent):
  """A TD3 Agent."""
...
#        clip=True)
    ...
    collect_policy = epsilon_discrete_boltzmann_policy.EpsilonDiscreteBoltzmannPolicy(
          policy, epsilon=self._epsilon_greedy, env_action_spec=env_action_spec)   
...
#    policy = collect_policy
    ...
    super(Td3DiscreteAgent, self).__init__(
        time_step_spec,
        env_action_spec,
        policy,
        collect_policy,
  ...
  def _train(self, experience, weights=None):
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      ...
      ## TO 
      actions_logits_begin = [0] + [0] * (len(self._replay_buffer_action_spec.shape)-1) + [1]
      actions_logits_end = [-1] + [-1] * (len(self._replay_buffer_action_spec.shape)-1) + [-1]
      action_logits = tf.slice(actions, begin=actions_logits_begin, size=actions_logits_end)
#      action_logits = actions[:, 1:]
      critic_loss = self.critic_loss(time_steps, action_logits, next_time_steps,
                                     weights=weights)
  ...
  def critic_loss(self, time_steps, actions, next_time_steps, weights=None):
    ...
    with tf.name_scope('critic_loss'):
      ...
      ##TODO: how to add noise to discrete action as input for critic network?
#      noisy_target_actions = tf.nest.map_structure(add_noise_to_action,
#                                                   target_actions)
      noisy_target_actions = target_actions
  ...
  def actor_loss(self, time_steps, weights=None):
    ...
    with tf.name_scope('actor_loss'):
      ##TODO: output from actor network is logits, should action be represented as logits or probs?
